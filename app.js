...
const defaults = {
  ...
  revenueCompletion:139, profitCompletion:152, battleCost:0, delegatedHours:0, delegatedRate:2000,
...
const K2 = {
  1:[1,1,1,1,1],
  2:[1.1,1.08,1.05,1.03,1],
  3:[1.4,1.3,1.15,1.1,1.03],
  4:[1.6,1.4,1.2,1.15,1.05],
  5:[1.6,1.5,1.3,1.2,1.1]
};
...
function readState(){
  ...
  state.revenueCompletion=num('revenueCompletion'); state.profitCompletion=num('profitCompletion');
...
function calculate(){
  ...
  const revComplete=state.revenueCompletion/100, profitComplete=state.profitCompletion/100;
  const revRate=revenueRewardRate(revComplete), profitRate=profitRewardRate(profitComplete);
  ...
  const managerTarget=managerStandard*.6*revRate+managerStandard*.4*profitRate+managerStandard*.4*scaleRate-totalCost;
  const shiftTarget=shiftStandard*.6*revRate+shiftStandard*.4*profitRate+shiftStandard*.4*scaleRate;
  ...
  const managerPool=managerTarget*managerFactor, shiftPool=shiftTarget*shiftFactor;

  const result={band,k1,k2,revRate,profitRate,scaleRate,totalCost,managerStandard,shiftStandard,managerTarget,shiftTarget,managerHours,shiftHours,managerFactor,shiftFactor,managerPool,shiftPool};
  renderResults(result); return result;
}
function renderResults(r){
  ...
  $('revenueRate').textContent=percent(r.revRate); $('profitRate').textContent=percent(r.profitRate); $('scaleRate').textContent=percent(r.scaleRate);
...
function renderAllocation(id,people,pool,totalHours){
  ...
  const active=people.filter(p=>p.hours>0);
  if(!active.length){box.innerHTML='<div class="empty">Chưa có giờ công.</div>';return;}
  active.forEach(p=>{
    const amount=totalHours?pool*p.hours/totalHours:0;
    const el=document.createElement('div'); el.className='allocation-item';
    el.innerHTML=`<div><b>${escapeHtml(p.name)}</b><div class="meta">${number.format(p.hours)} giờ · ${percent(totalHours?p.hours/totalHours:0)}</div></div><strong>${currency.format(amount)}</strong>`;
    box.appendChild(el);
  });
}
...
function renderPeople(type){
  ...
  box.innerHTML=''; people.forEach((p,i)=>{
    const row=document.createElement('div'); row.className='person-row';
    row.innerHTML=`<input class="person-name" aria-label="Họ tên" value="${escapeHtml(p.name)}"><input class="person-hours" aria-label="Giờ công" type="number" min="0" step="0.5" value="${p.hours}"><button class="icon-btn" title="Xóa" data-remove="${type}" data-index="${i}">×</button>`;
    box.appendChild(row);
  });
}
function fillForm(){
...
function download(name,content,type){const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([content],{type}));a.download=name;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(a.href);}
...
document.addEventListener('input',e=>{if(e.target.matches('input,select'))calculate();});
document.addEventListener('change',e=>{if(e.target.matches('select'))calculate();});
document.addEventListener('click',e=>{
  const add=e.target.dataset.add;if(add){const list=add==='manager'?state.managers:state.shifts;readState();list.push({name:add==='manager'?`Quản lý ${list.length+1}`:`Trưởng ca ${list.length+1}`,hours:0});if(add==='manager')state.managers=list;else state.shifts=list;renderPeople(add);calculate();}
...
$('saveBtn').onclick=save;$('resetBtn').onclick=()=>{if(confirm('Khôi phục toàn bộ dữ liệu mẫu?')){state=structuredClone(defaults);localStorage.removeItem('bonusCalculatorData');fillForm();}};$('printBtn').onclick=()=>window.print();$('exportJsonBtn').onclick=exportJson;$('exportCsvBtn').onclick=exportCsv;
